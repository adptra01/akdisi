...
get_template_part( 'template-parts/page-hero', null, array(
	'eyebrow' => __( 'Contact', 'akdisi' ),
	'title'   => __( "Let's talk about your project.", 'akdisi' ),
	'sub'     => __( 'Tell us where you want to go and a senior team member will reply within one business day.', 'akdisi' ),
) );
?>
...
<section class="section section-bg">
	<div class="container mx-auto grid items-start gap-12 lg:grid-cols-[0.9fr_1.1fr]">
		<!-- Info -->
		<div data-reveal>
			<p class="eyebrow mb-4">Reach us</p>
			<h2 class="section-title">Prefer a quicker route?</h2>
			...
			<div class="mt-10 rounded-2xl border border-paper-line bg-paper-alt p-6">
				<p class="text-sm leading-relaxed text-ink-soft">
					<strong class="text-ink">Office hours:</strong> Mon–Fri, 09.00–17.00 WIB.
					Quick requests usually get a same-day reply.
				</p>
			</div>
		</div>
		...
		<div data-reveal class="rounded-2xl border border-paper-line bg-white p-7 shadow-[var(--shadow-card)] md:p-10">
			<form id="akdisi-contact-form" class="grid gap-5" novalidate>
				<div class="grid gap-5 sm:grid-cols-2">
					<div>
						<label for="f-name" class="mb-1.5 block text-sm font-medium text-ink">Name</label>
						...
						<p class="akdisi-field-error mt-1 hidden text-xs text-brand">Please enter your name.</p>
					</div>
					<div>
						<label for="f-email" class="mb-1.5 block text-sm font-medium text-ink">Email</label>
						<input id="f-email" name="email" type="email" required autocomplete="email"
							class="w-full rounded-lg border border-paper-line bg-white px-4 py-3 text-sm text-ink placeholder:text-ink-faint focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20">
						<p class="akdisi-field-error mt-1 hidden text-xs text-brand">Please enter a valid email.</p>
					</div>
				</div>
				<div>
					<label for="f-company" class="mb-1.5 block text-sm font-medium text-ink">Company <span class="text-ink-faint">(optional)</span></label>
					<input id="f-company" name="company" type="text"
						class="w-full rounded-lg border border-paper-line bg-white px-4 py-3 text-sm text-ink placeholder:text-ink-faint focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20">
				</div>
				<div>
					<label for="f-budget" class="mb-1.5 block text-sm font-medium text-ink">Budget range</label>
					<select id="f-budget" name="budget"
						...
						<option>Under Rp 25 juta</option>
				...
				<div>
					<label for="f-message" class="mb-1.5 block text-sm font-medium text-ink">Tell us about the project</label>
					...
					<p class="akdisi-field-error mt-1 hidden text-xs text-brand">Please describe your project briefly.</p>
				</div>
